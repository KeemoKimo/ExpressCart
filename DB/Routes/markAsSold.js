...
router.post('/markAsSold', async (req, res) => {
    try {
        ...
        const updateQuery = `UPDATE items SET is_sold = TRUE, solddate = NOW() WHERE id = $1`;
        ...
        if (updateQueryResult.rowCount > 0) {
            ...
            res.json({ success: true, message: "Item marked as sold!", redirectUrl: `/mainPage?userName=${userName}` });
        } else {
